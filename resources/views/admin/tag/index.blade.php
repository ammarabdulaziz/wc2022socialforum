...
<!-- <link rel="stylesheet" href="https://cdn.datatables.net/1.10.12/css/dataTables.bootstrap.min.css" />
	...
	<section class="content">
		<div class="box">
			<div class="box-header with-border">
				...
				<div class="box-tools">
					<button type="button" class="btn btn-info btn-sm btn-flat add-button"><i class="fa fa-plus"></i> Add Tag</button>
				</div>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
				<div style="width: 100%; padding-left: -10px;">
					<div class="table-responsive">
						<table id="tags-table" class="table table-striped table-hover dt-responsive display nowrap" cellspacing="0">
							<thead>
								<tr>
									<th>Tag Name</th>
									<th>Tag Slug</th>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<form role="form" id="tag_add_form" method="post">
						{{ csrf_field() }}
						<div class="modal-body">
							<div class="form-group">
								<label for="tag_name">Tag Name</label>
								<input type="text" name="tag_name" class="form-control" id="tag_name" value="{{ old('tag_name') }}" placeholder="ex: tag">
								<span class="text-danger" id="tag-name-error"></span>
							</div>
							<div class="form-group">
								<label for="tag_slug">Tag Slug</label>
								<input type="text" name="tag_slug" class="form-control" id="tag_slug" value="{{ old('tag_slug') }}" placeholder="ex: tag_slug">
								<span class="text-danger" id="tag-slug-error"></span>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<input type="text" name="meta_title" class="form-control" id="meta_title" value="{{ old('meta_title') }}" placeholder="ex: tag title">
								<span class="text-danger" id="meta-title-error"></span>
							</div>
							<div class="form-group">
								<label for="meta_keywords">Meata Keywords</label>
								<input type="text" name="meta_keywords" class="form-control" id="meta_keywords" value="{{ old('meta_keywords') }}" placeholder="ex: tag, title">
								<span class="text-danger" id="meta-keywords-error"></span>
							</div>
							<div class="form-group">
								<label for="meta_description">Meta Description</label>
								<textarea name="meta_description" class="form-control" id="meta_description" rows="3" placeholder="ex: tag dscription">{{ old('meta_description') }}</textarea>
				...
				<div class="modal-dialog modal-lg" role="document">
					<div class="modal-content">
						<div class="modal-header">
							...
							<form role="form" id="tag_edit_form" method="post">
								{{method_field('PATCH')}}
								{{csrf_field()}}
								...
								<div class="modal-body">
									<div class="form-group">
										<label for="tag_name">Tag Name</label>
										<input type="text" name="tag_name" class="form-control" id="edit-tag-name" placeholder="ex: tag">
										<span class="text-danger tag-name-error"></span>
									</div>
									<div class="form-group">
										<label for="tag_slug">Tag Slug</label>
										<input type="text" name="tag_slug" class="form-control" id="edit-tag-slug" placeholder="ex: tag_slug">
										<span class="text-danger tag-slug-error"></span>
									</div>
									<div class="form-group">
										<label>Publication Status</label>
									...
									<div class="form-group">
										<label for="meta_title">Meata Title</label>
										<input type="text" name="meta_title" class="form-control" id="edit-meta-title" placeholder="ex: tag title">
										<span class="text-danger meta-title-error"></span>
									</div>
									<div class="form-group">
										<label for="meta_keywords">Meata Keywords</label>
										<input type="text" name="meta_keywords" class="form-control" id="edit-meta-keywords" placeholder="ex: tag, title">
										<span class="text-danger meta-keywords-error"></span>
									</div>
									<div class="form-group">
										<label for="meta_description">Meta Description</label>
										<textarea name="meta_description" class="form-control" id="edit-meta-description" rows="3" placeholder="ex: tag dscription"></textarea>
	...
	<script type="text/javascript">
		...
		/** Edit **/
		$("#tags-table").on("click", ".edit-button", function(){
			var tag_id = $(this).data("id");
			var url = "{{ route('admin.tags.show', 'tag_id') }}";
			...
			$.ajax({
				...
				success:function(data){
					...
					$('#edit-tag-slug').val(data['tag_slug']);
		...
		$(".update-button").click(function(){
			var tag_id = $('#edit-tag-id').val();
			var url = "{{ route('admin.tags.update', 'tag_id') }}";
			url = url.replace("tag_id", tag_id);
			var tag_edit_form = $("#tag_edit_form");
			var form_data = tag_edit_form.serialize();
			$( '.tag-name-error' ).html( "" );
			$( '.tag-slug-error' ).html( "" );
				...
				success:function(data) {
					...
					if(data.errors) {
						if(data.errors.tag_name){
							$( '.tag-name-error' ).html( data.errors.tag_name[0] );
						}
						if(data.errors.tag_slug){
							$( '.tag-slug-error' ).html( data.errors.tag_slug[0] );
						}
		...
		/** Delete **/
		$("#tags-table").on("click", ".delete-button", function(){
			var tag_id = $(this).data("id");
			var url = "{{ route('admin.tags.destroy', 'tag_id') }}";
			url = url.replace("tag_id", tag_id);
		...
		/** View **/
		$("#tags-table").on("click", ".view-button", function(){
			var tag_id = $(this).data("id");
			var url = "{{ route('admin.tags.show', 'tag_id') }}";
			url = url.replace("tag_id", tag_id);
			$.ajax({
				...
				success:function(data){
					...
					$('#view-tag-name').text(data['tag_name']);
					$('#view-tag-slug').text(data['tag_slug']);
		...
		$("#store-button").click(function(){
			var tag_add_form = $("#tag_add_form");
			var form_data = tag_add_form.serialize();
			$( '#tag-name-error' ).html( "" );
			$( '#tag-slug-error' ).html( "" );
			$( '#publication-status-error' ).html( "" );
			$( '#meta-title-error' ).html( "" );
		...
		/** User View **/
		$("#tags-table").on("click", ".user-view-button", function(){
			var id = $(this).data("id");
