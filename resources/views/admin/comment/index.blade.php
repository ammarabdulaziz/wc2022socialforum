...
<!-- <link rel="stylesheet" href="https://cdn.datatables.net/1.10.12/css/dataTables.bootstrap.min.css" />
			...
			<div class="box-body">
				<div style="width: 100%; padding-left: -10px;">
					<div class="table-responsive">
						<table id="comments-table" class="table table-striped table-hover dt-responsive display nowrap" cellspacing="0">
							<thead>
								<tr>
									<th>Post Title</th>
									<th>Comment By</th>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="view-comment-name"></h4>
				</div>
				<div class="modal-body">
					<table class="table table-bordered table-striped">
						<tbody>
							<tr>
								<td>Post title</td>
								<td id="view-post-title"></td>
							</tr>
							<tr>
								<td>Comment By</td>
								<td id="view-comment-by"></td>
							</tr>
							<tr>
								<td>Comment</td>
								<td id="view-comment"></td>
							</tr>
							<tr>
								<td>Created At</td>
	...
	<script type="text/javascript">
		...
		/** Delete **/
		$("#comments-table").on("click", ".delete-button", function(){
			var comment_id = $(this).data("id");
			var url = "{{ route('admin.comments.destroy', 'comment_id') }}";
		...
		/** View **/
		$("#comments-table").on("click", ".view-button", function(){
			var comment_id = $(this).data("id");
			var url = "{{ route('admin.comments.show', 'comment_id') }}";
			url = url.replace("comment_id", comment_id);
			$.ajax({
				url: url,
				method: "GET",
				...
				success:function(data){
					...
					$('#view-post-title').text(data['post']['post_title']);
					$('#view-comment-by').text(data['user']['name']);
					$('#view-comment').text($(data['comment']).text());
		...
		function get_table_data(){
			$('#comments-table').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('admin.getCommentsRoute') }}",
				columns: [
				{data: 'post', name: 'post', orderable: true, searchable: true},
		...
		/** User View **/
		$("#comments-table").on("click", ".user-view-button", function(){
			var id = $(this).data("id");
		...
		/** Post View **/
		$("#comments-table").on("click", ".post-view-button", function(){
